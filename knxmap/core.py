...
class KnxMap(object):
    ...
    async def _search_gateways(self):
        self.t0 = time.time()
        await asyncio.ensure_future(self._knx_search_worker())
        self.t1 = time.time()
    ...
    async def brute(self, targets=None, bus_target=None, full_key_space=False):
        if targets:
            self.set_targets(targets)
        tasks = [asyncio.create_task(self.bruteforce_auth_key(t, bus_target, full_key_space))
    ...
    async def _bus_scan(self, knx_gateway, bus_targets):
        ...
        connectors = [asyncio.create_task(self._tunnel_connection(knx_gateway))
        ...
        LOGGER.info('Established %d connections to target %s' %
                    (len(self.bus_connections[knx_gateway.host]),
                     knx_gateway.host))
        workers = [asyncio.create_task(self._knx_bus_worker(c.get('transport'),
                                                     c.get('protocol'),
                                                     knx_gateway))
    ...
    async def scan(self, targets=None, desc_timeout=2, desc_retries=2, bus_timeout=2,
        ...
        if targets:
            self.set_targets(targets)
        if self.medium == 'net':
            workers = [asyncio.create_task(self._knx_description_worker())
            ...
            if bus_targets and self.knx_gateways:
                ...
                bus_scanners = [asyncio.create_task(self._bus_scan(knx_gateway=g,
